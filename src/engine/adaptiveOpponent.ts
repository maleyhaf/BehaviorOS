...
const RULES: Rule[] = [
  {
    ...
    apply: (m, p) => ({ decoyProbability: Math.min(m.decoyProbability + 0.08, 0.4) }),
  },
  {
    ...
    apply: (m, p) => ({ decoyDamageMultiplier: Math.min(m.decoyDamageMultiplier + 0.3, 2.0) }),
  },
  {
    id: 'patient_pressure',
    ...
    apply: (m, p) => ({ decaySpeed: Math.max(m.decaySpeed - 0.1, 0.5) }),
  },
  {
    ...
    apply: (m, p) => ({ missDamageMultiplier: Math.min(m.missDamageMultiplier + 0.4, 2.5) }),
  },
  {
    id: 'risk_traps',
    ...
    apply: (m, p) => ({ decoyProbability: Math.min(m.decoyProbability + 0.05, 0.4) }),
  },
  {
    ...
    apply: (m, p) => ({ rewardScaling: Math.max(m.rewardScaling - 0.1, 0.5) }),
  },
  {
    ...
    apply: (m, p) => ({ growthSpeed: Math.min(m.growthSpeed + 0.15, 2.0) }),
  },
  {
    id: 'spawn_pressure',
    ...
    apply: (m, p) => ({ spawnRate: Math.min(m.spawnRate + 0.2, 2.5) }),
  },
  {
    id: 'patience_drift',
    ...
    apply: (m, p) => ({ driftSpeed: Math.min(m.driftSpeed + 35, 80) }),
  },
  {
    ...
    apply: (m, p) => ({ driftSpeed: Math.min(m.driftSpeed + 30, 130) }),
  },
  {
